...
export function parseOBJ(source, options = {}) {
  ...
  for (const rawLine of source.split(/\r?\n/)) {
    ...
    if (command === "v") {
      ...
      if (![x, y, z].every(Number.isFinite)) throw new Error(`Invalid OBJ vertex: ${line}`);
      vertices.push({ x, y, z });
    }

    if (command === "f") {
      if (values.length < 3) throw new Error(`Invalid OBJ face: ${line}`);
      const polygon = values.map((token) => resolveIndex(token, vertices.length));
      if (polygon.some((index) => index < 0 || index >= vertices.length)) {
